            ...
            if (!$r->isFinal()) {
                foreach ($r->getMethods() as $key => $method) {
                    if ($method->isPublic() && !$method->isConstructor() && !$method->isFinal()) {
                        ...
                        $this->add('', $r->name . "::" . $method->name);

                        if ($_SERVER['REQUEST_URI'] == '/articles')
                            return $this->add('articles', $r->name . "::" . $method->name);

                        if ($_SERVER['REQUEST_URI'] == '/article/'.basename($_SERVER['REQUEST_URI'])){
                            return $this->add('article/{$slug}', $r->name . "::" . $r->getMethods()[1]->name);
                        }
